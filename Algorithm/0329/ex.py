...
def crossover(p):           # 用單點交配來繁衍子代
    ...
    for i in range(NUM_CROSSOVER) :
        ...
        d = np.random.randint(c, NUM_BIT)               #njo6[
        [j, k] = np.random.choice(NUM_PARENT, 2, replace=False)  # 任選兩個index
       
        a.append(np.concatenate((p[j][0: c], p[k][c: d], p[j][d: NUM_BIT]), axis=0))
        a.append(np.concatenate((p[k][0: c], p[j][c: d], p[k][d: NUM_BIT]), axis=0))
